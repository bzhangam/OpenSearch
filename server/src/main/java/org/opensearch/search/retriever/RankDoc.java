 ...
 * pinning, etc.) and ships it to every shard as part of the internal {@code RankDocsQuery}. {@code _id}
...
public final class RankDoc implements Writeable, ToXContentObject {
     ...
     * @param score    the pre-computed retriever score for the document; MUST be non-negative and non-NaN
    ...
    public RankDoc(StreamInput in) throws IOException {
        ...
        // Guard the wire path too: a malformed/hostile stream must not smuggle a negative or NaN score
        // past the same invariant the public constructor enforces.
        this.score = validateScore(in.readFloat());
        this.position = in.readVInt();
    }

    /**
     * The retriever replays a score the coordinator already computed; a negative or NaN score is never a
     * valid replayed relevance value, and it would silently corrupt the {@code score * boost} path in
     * {@code RankDocsQuery}. Reject it at this single source of truth so every downstream consumer is safe.
     */
    private static float validateScore(float score) {
        if (score < 0f || Float.isNaN(score)) {
            throw new IllegalArgumentException("score must be non-negative and non-NaN, got [" + score + "]");
        }
        return score;
    }
